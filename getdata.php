...
$d = date("Y-m-d");
$t = date("H:i:s"); // Removed 'a' for standard time format

if (isset($_GET['card_uid']) && isset($_GET['device_token'])) {
    
    $card_uid = strtoupper(trim($_GET['card_uid'])); // Convert to uppercase and trim whitespace
    $device_uid = trim($_GET['device_token']);

    // Log incoming data for debugging
    file_put_contents('debug_log.txt', "Received card_uid: $card_uid, device_token: $device_uid\n", FILE_APPEND);
    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($device_mode == 1) { // Login/Logout mode
            ...
            } else if ($device_mode == 0) { // New card mode
                $sql = "SELECT * FROM users WHERE card_uid=?";
                $result = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($result, $sql)) {
                    echo "SQL_Error_Select_card";
